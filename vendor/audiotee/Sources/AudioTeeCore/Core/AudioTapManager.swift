...
public class AudioTapManager {
  ...
  private func createSystemAudioTap(with config: TapConfiguration) throws -> AudioObjectID {
    ...
    let processObjects = try translatePIDsToProcessObjects(config.processes)

    let description: CATapDescription
    if config.isExclusive {
      description =
        config.isMono
        ? CATapDescription(monoGlobalTapButExcludeProcesses: processObjects)
        : CATapDescription(stereoGlobalTapButExcludeProcesses: processObjects)
    } else if processObjects.isEmpty {
      description =
        config.isMono
      ...
      description =
        config.isMono
        ? CATapDescription(monoMixdownOfProcesses: processObjects)
        : CATapDescription(stereoMixdownOfProcesses: processObjects)
    }
